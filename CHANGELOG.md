...
### Fixed
- Fixed extraction failing with `tar exit 141` (SIGPIPE) in rooted/chroot mode.
  The extractor runs as root via `su -c`, and piping the decompressed stream into
  `su -c tar` broke because the stdin pipe closed (the decompressor died with
  SIGPIPE). In chroot mode the archive is now decompressed to a temporary file and
  extracted from that file, so no pipe crosses the `su` boundary. Non-root modes
  keep the direct pipe. The temporary file is removed afterward.

## [1.2.2] - 2026-08-30

### Fixed
- Cleaning/removing a root-owned rootfs no longer fails with "Permission denied".
  A rootfs extracted in chroot mode is owned by root, so a non-root shell could not
  delete it (e.g. `var/spool/mail`, `var/tmp`), aborting reinstall/restore/uninstall
  with "Could not clean previous rootfs". `safe_remove` now tries the current user
  first and, when allowed (`use_root`), escalates to root to finish the removal.
  Applied to install cleanup, restore, and uninstall.
